...
impl Authenticator {
    ...
    pub fn refresh_token(&self, token: &str) -> Result<String, AuthError> {
        ...
        let roles: Vec<Role> = claims.roles.iter().map(|r| Role::new(r)).collect();
...
mod tests {
    ...
    fn test_expired_token() {
        ...
        let token = auth.generate_token_with_expiry(&user_id, &roles, Duration::seconds(-1));
