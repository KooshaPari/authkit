...
use crate::domain::{
    User, UserId, Role, Session, SessionId, Claims, Authenticator,
    PolicyEngine, UserStorage, SessionStorage, PasswordHasher,
    errors::AuthError,
};
...
impl AuthService {
    ...
    pub async fn register(
        ...
        if self.user_storage.get_by_email(&email).await
            .map_err(|e| AuthError::StorageError(e))?
            .is_some()
        {
    ...
    pub async fn login(
        ...
        // Get user
        let user = self.user_storage.get_by_email(email).await
            .map_err(|e| AuthError::StorageError(e))?
    ...
    pub async fn logout(&self, session_id: &SessionId) -> Result<(), AuthError> {
        let mut session = self.session_storage.get_by_id(session_id).await
            .map_err(|e| AuthError::StorageError(e))?
            .ok_or(AuthError::SessionNotFound)?;

        session.revoke();

        self.session_storage.update(&session).await
            .map_err(|e| AuthError::StorageError(e))?;
    ...
    pub async fn get_user(&self, user_id: &UserId) -> Result<Option<User>, AuthError> {
        self.user_storage.get_by_id(user_id).await
            .map_err(|e| AuthError::StorageError(e))
    }
}
