...
use crate::domain::{User, UserId, Session, SessionId};
use crate::domain::ports::{UserStorage, SessionStorage};
...
impl SessionStorage for InMemorySessionStorage {
    async fn create(&self, session: &Session) -> Result<(), String> {
        ...
        by_user.entry(session.user_id.clone())
            .or_default()
